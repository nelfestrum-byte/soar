...
**Обновление (B1, 2026-07-28):** до фикса `docs/compose/specs/2026-07-28-auth-deactivation-revocation-design.md`
деактивация/разжалование B через API не отзывала уже выданный B refresh-токен —
"self-lockout" A над B был неполным: B мог продолжать получать новые
access-токены через `/auth/refresh` неограниченно. Теперь `update_user()`/
`set_user_active()` отзывают все живые refresh-токены цели в той же
транзакции — сценарий self-lockout в этом описании работает так, как здесь
описано с самого начала. Сам принятый риск (потеря последнего работающего
admin'а и recovery только через CLI) не изменился.
