...
deploy/prod/                   # Distributable profile — image: не build:, config.yaml не в git
├── docker-compose.yml         # ${SOAR_VERSION}/${POSTGRES_PASSWORD}/${SOAR_WEBHOOK_TOKEN} из .env, config.yaml bind-mount
├── Dockerfile.orchestrator    # как stage, но без COPY config.yaml (не запекается в образ)
├── Dockerfile.ui               # как stage
├── nginx.conf
├── config.yaml.template        # ${AUTH_SECRET_KEY}/${POSTGRES_PASSWORD} — рендерится soarctl init, не docker compose
└── README.md                   # package → install → init → up → migrate → users create --admin

deploy/soarctl                  # Host-layer CLI (thin argparse-обёртка, stdlib-only)
deploy/soarctl_lib/             # package/install/init/up/down/status/migrate/users/backup/doctor
├── runner.py                   # единственная точка вызова subprocess
├── bundle.py                   # package (build machine) / install (target, offline)
├── env.py                      # генерация .env + рендер config.yaml.template
├── compose.py, migrate.py, users.py, backup.py, doctor.py, status.py, paths.py
└── cli.py                      # argparse-диспетчер (deploy/soarctl — тонкий shim поверх него)
