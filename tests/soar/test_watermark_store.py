...
def test_watermark_set_get_roundtrip(tmp_path):
    store = WatermarkStore(str(tmp_path / "wm.json"))
    store.set("source_a", "2026-07-05T10:40:00+00:00")
    assert store.get("source_a") == "2026-07-05T10:40:00+00:00"


def test_watermark_survives_restart(tmp_path):
    path = str(tmp_path / "wm.json")
    WatermarkStore(path).set("source_a", "2026-07-05T10:40:00+00:00")
    # new instance, same file — simulates orchestrator restart
    assert WatermarkStore(path).get("source_a") == "2026-07-05T10:40:00+00:00"


def test_watermark_independent_keys(tmp_path):
    store = WatermarkStore(str(tmp_path / "wm.json"))
    store.set("source_a", "2026-07-05T10:00:00+00:00")
    store.set("source_b", "2026-07-05T11:00:00+00:00")
    assert store.get("source_a") == "2026-07-05T10:00:00+00:00"
    assert store.get("source_b") == "2026-07-05T11:00:00+00:00"
...
def test_seen_store_mark_and_check(tmp_path):
    ...
    assert store.is_seen("seen:1") is False
    store.mark("seen:1")
    assert store.is_seen("seen:1") is True
    # persists across instances
    assert SeenStore(str(tmp_path / "seen.json")).is_seen("seen:1") is True
...
def test_seen_store_ttl_expiry(tmp_path):
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump({"seen:1": time.time() - 10}, f)
    assert store.is_seen("seen:1") is False
